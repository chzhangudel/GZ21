...
def load_paper_net(device: str = 'gpu',file_name='trained_model_global_BC_RP2'):
    ...
    print(net)
    return net
net1 = load_paper_net('cpu',file_name='trained_model_global_BC_8505')
net1.eval()
net2 = load_paper_net('cpu',file_name='trained_model_global_BC_9593')
net2.eval()
net3 = load_paper_net('cpu',file_name='trained_model_global_BC_3351')
net3.eval()
net4 = load_paper_net('cpu',file_name='trained_model_global_BC_6770')
net4.eval()
...
test2 = create_large_test_dataset(net2.to(device=device), criterion, [dataset, ], [DataLoader(dataset)], device,mask=False, replicate=False)
test2 = test2.rename(dict(longitude='xu_ocean', latitude='yu_ocean'))
test3 = create_large_test_dataset(net3.to(device=device), criterion, [dataset, ], [DataLoader(dataset)], device,mask=False, replicate=False)
test3 = test3.rename(dict(longitude='xu_ocean', latitude='yu_ocean'))
test4 = create_large_test_dataset(net4.to(device=device), criterion, [dataset, ], [DataLoader(dataset)], device,mask=False, replicate=False)
test4 = test4.rename(dict(longitude='xu_ocean', latitude='yu_ocean'))
...
test1.to_netcdf('test_global_nobc_FCNN_g1.nc')
test2.to_netcdf('test_global_nobc_FCNN_g2.nc')
test3.to_netcdf('test_global_nobc_FCNN_g3.nc')
test4.to_netcdf('test_global_nobc_FCNN_g4.nc')