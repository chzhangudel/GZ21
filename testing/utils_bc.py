...
def apply_net(net, test_dataloader, device, save_input=False,mask=False, replicate=False):
    """
    ...
    with torch.no_grad():
        for i, data in enumerate(test_dataloader):
            ...
            if features_pad.shape[1] == 3:
                features_pad[:,0,:,:] =  features_pad[:,0,:,:]*features_pad[:,2,:,:]
                features_pad[:,1,:,:] =  features_pad[:,1,:,:]*features_pad[:,2,:,:]
                if mask is True:
                    maskn = features_pad[:,2,:,:]
                    maskn = maskn.unsqueeze(1)
                    maskn = torch.where(maskn == 0, torch.tensor(float('nan')), maskn)
                else:
                    maskn = None
            prediction = (net(features_pad[:,:2,:,:],maskn=maskn, replicate=replicate)).cpu().numpy()
...
def create_large_test_dataset(net, criterion, test_datasets, test_loaders,
    ...
    for i, loader in enumerate(test_loaders):
        ...
        temp = delayed_apply(net, loader, device,mask=mask, replicate=replicate)
        # temp = apply_net(net, loader, device,mask=False, replicate=False)
